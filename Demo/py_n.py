...
class Crop:
    ...
    def update_daily_outputs(self, row):
        self.df_daily_outputs.loc[row, :] = [self.scenario_name, self.location_name, self.manage_name,
                                             ...
                                             self.DAP, self.TMP, self.DTU, self.CTU, self.NDS, self.LAI,
                                             self.TCFRUE, self.FINT, self.DDMP, self.SGR, self.WVEG,
                                             self.WGRN, self.WTOP, self.HI, self.DEPORT, self.RAIN, self.IRGW,
                                             self.RUNOF, self.PET, self.SEVP, self.TR, self.TR1, self.DRAIN, self.DRAIN1,
                                             self.ATSW, self.ATSW1, self.FTSW, self.CRAIN, self.CIRGW, self.IRGNO,
                                             ...
                                             self.NGRN, self.CNUP, self.CUMBNF, self.WAT1, self.NCON]
    ...
    def update_summary_outputs(self, row):
        ...
        self.CSEVP = float(self.df_daily_outputs["SEVP"].sum())
        self.CPET = float(self.df_daily_outputs["PET"].sum())
        self.CNSOL = float(self.df_daily_outputs["NSOL"].sum())
        self.df_summary_outputs.loc[row, :] = [self.scenario_name, self.location_name, self.manage_name,
                                               ...
                                               self.dtBSG, self.dtTSG, self.dtHAR, self.WTOP, self.WGRN, self.WVEG,
                                               self.HI99, self.ISOLWAT, self.CRAIN, self.CIRGW, self.IRGNO,
                                               self.ATSW, self.CRUNOF, self.CE, self.CTR, self.CSEVP, self.CPET, self.WSTORG,
                                               ...
                                               self.CNUP, self.CNSOL, self.CUMBNF, self.INSOL, self.CNFERT, self.CNMIN,
    ...
    def gen_daily_graphs(self, year=None):
        ...
        name_dict = {"CTU": {"name": "Cumulative Temperature Unit", "unit": "oC"},
                     "NDS": {"name": "NDS", "unit": "ratio, nu"},
                     ...
                     "HI": {"name": "Harvest Index x100", "unit": "nu"},
                     "ATSW": {"name": "Avaiable Soil Water", "unit": "mm"},
                     "RAIN": {"name": "Rainfall", "unit": "mm"},
                     "IRGW": {"name": "Irrigation Water", "unit": "mm"},
                     "WSTORG": {"name": "Soil Water Storage", "unit": "mm"},
                     "ATSW1" : {"name": "Available Soil Water Layer 1", "unit": "mm"},
                     "RUNOF": {"name": "Runoff", "unit": "mm"},
                     "DRAIN1": {"name": "Drainage Layer 1", "unit": "mm"},
                     "TR1": {"name": "Transpiration Layer 1", "unit": "mm"},
                     "CTR": {"name": "Cumulative Transpiration", "unit": "mm"},
                     "CE": {"name": "Cumulative Soil Evaporation", "unit": "mm"},
                     "CRAIN": {"name": "Cumulative Rain", "unit": "mm"},
                     "CRUNOF": {"name": "Cumulative Runoff", "unit": "mm"},
                     "CIRGW": {"name": "Cumulative Irrigation Water", "unit": "mm"},
                     "TR": {"name": "Actual Transpiration", "unit": "mm"},
                     "SEVP": {"name": "Actual Soil Evaporation", "unit": "mm"},
                     "PET": {"name": "Potential Evapotranspiration", "unit": "mm"},
                     "NST": {"name": "Total Nitrogen in Stems", "unit": "g N m-2"},
                     "NLF": {"name": "Total Nitrogen in Leaves", "unit": "g N m-2"},
                     "NVEG": {"name": "Total Nitrogen Above Ground Plant", "unit": "g N m-2"},
                     "NGRN": {"name": "Total Nitrogen in Grain", "unit": "g N m-2"},
                     "CNUP": {"name": "Cumulative Nitrogen Uptake", "unit": "g N m-2"},
                     "CNMIN": {"name": "Cumulative Net Mineralization", "unit": "g N m-2 d-1"},
                     "NVOL": {"name": "Nitrogen Volatilization", "unit": "g N m-2 d-1"},
                     "NLEACH": {"name": "Nitrogen Leaching Loss", "unit": "g N m-2 d-1"},
                     "WAT1": {"name": "Water Layer 1", "unit": "mm"},
                     "NDNIT": {"name": "Denitrification", "unit": "g N g-1 H2O day-1"},
                     "MNORG": {"name": "Mineralizable Organic Nitrogen", "unit": "g N m-2"},
                     "CNFERT": {"name": "Cumulative Total Fertilizer Applied", "unit": "g N m-2"},
                     "CNVOL": {"name": "Cumulative Total Nitrogen Volatilized", "unit": "g N m-2 d-1"},
                     "CNLEACH": {"name": "Cumulative Nitrogen Leaching", "unit": "g N m-2 d-1"},
                     "CNDNIT": {"name": "Cumulative Denitrification", "unit": "g N g-1 H20 day-1"},
                     "NSOL": {"name": "Soluble (Available) Nitrogen", "unit": "g N m-2"},
                     "NMIN": {"name": "Net Mineralization", "unit": "g N m-2"},
                     "NFERT": {"name": "Fertilizer Applied", "unit": "g N m-2 d-1"},
                     "NUP": {"name": "Nitrogen Uptake", "unit": "g N m-2"},
                     "NCON": {"name": "NCON", "unit": "g N g-1 H2O"}}

        
        rename_dict = {"TR": "TRa", "SEVP": "SEVPa"}
        graph_y_vars = ['CTU', 'NDS', 'LAI', ['DDMP', 'SGR'], ['TR', 'SEVP', 'PET'],
                        ['ATSW', 'RAIN', 'IRGW', 'WSTORG'],
                        ['ATSW1', 'IRGW'],
                        ['RAIN', 'RUNOF', 'DRAIN1', 'SEVP', 'TR1'],
                        ['CTR', 'CE', 'CRAIN', 'CRUNOF', 'CIRGW'],
                        ['WVEG', 'WGRN', 'HI'],
                        ['NSOL', 'NFERT'],
                        ['NMIN', 'NVOL', 'NLEACH', 'NDNIT', 'NUP'],
                        ['NCON', 'NSOL', 'WAT1'],
                        ['CNMIN', 'CNFERT', 'CNVOL', 'CNLEACH', 'CNDNIT'],
                        ['NST', 'NLF', 'NVEG', 'NGRN', 'CNUP']]
        for y_var in graph_y_vars:
                xaxis_title = "Days After Planting (Planting date: {})".format(self.Pdoy)
                #xaxis_title = 'Day of the Year (Julian)'
                if isinstance(y_var, list):
                    yname = '_'.join(y_var)
                    title_name = ' &<br> '.join(["{} ({}, {})".format(name_dict.get(x).get("name"),
                                                                  x,
                                                                  name_dict.get(x).get("unit")) for x in y_var])
                    
                    title = 'Scenario: {}<br>Simulation Year: {}'.format(self.scenario_name, self.Pyear)
                    yaxis_title = '{}'.format(title_name)
                    data = []
                    for y in y_var:
                        y_graph = df[y].values.tolist()
                        if y in list(rename_dict.keys()):
                            trace_name = rename_dict.get(y)
                        else:
                            trace_name = y
                        if y == "HI":
                            y_graph = [i*100 for i in y_graph]
                        trace_temp = go.Scatter(x=x_graph, y=y_graph,
                                                mode='lines+markers',
                                                name='{}'.format(trace_name))
                    ...
                    title = 'Scenario: {}<br>Simulation Year: {}'.format(self.scenario_name, self.Pyear)
                    yaxis_title = '{} ({}, {})'.format(tname, yname, tunit)
                    data = []
                    y_graph = df[y_var].values.tolist()
                    if y_var in list(rename_dict.keys()):
                        trace_name = rename_dict.get(y_var)
                    else:
                        trace_name = y_var
                    if y_var == "HI":
                        y_graph = [i*100 for i in y_graph]
    ...
    def gen_summary_graphs(self):
        ...
        if len(x_graph) > 1:
            sim_y = "{}-{}".format(x_graph[0], x_graph[-1])
        else:
            sim_y = "{}".format(x_graph[0])
        name_dict = {"WGRN": {"name": "Accumulated Grain", "unit": "g m-2"},
                     "WVEG": {"name": "Accumulated Vegetation", "unit": "g m-2"},
                     "HI99": {"name": "Harvest Index x100", "unit": "nu"},
                     "WTOP": {"name": "Accumulated Crop", "unit": "g m-2"},
                     "CSEVP": {"name": "Accumulated Soil Evaporation", "unit": "mm"},
                     "CPET": {"name": " Accumulated Potential Evapotranspiration", "unit": "mm"},
                     "TE99": {"name": "Transpiration Efficency", "unit": "mm"},
                     "ET99": {"name": "Evapotranspiration", "unit": "mm"},
                     "EET99": {"name": "E/ET Ratio", "unit":"Ratio"},
                     "ATSW": {"name": "Avaiable Soil Water", "unit": "mm"},
                     "CRAIN": {"name": "Accumulated Rainfall", "unit": "mm"},
                     "CIRGW": {"name": "Accumulated Irrigation Water", "unit": "mm"},
                     "WSTORG": {"name": "Soil Water Storage", "unit": "mm"},
                     "CTR": {"name": "Accumulated Transpiration", "unit": "mm"},
                     "CE": {"name": "Accumulated Soil Evaporation", "unit": "mm"},
                     "CRUNOF": {"name": "Accumulated Runoff", "unit": "mm"},
                     "CNMIN": {"name": "Accumulated Net Mineralization", "unit": "g N m-2 d-1"},
                     "CNFERT": {"name": "Accumulated Total Fertilizer Applied", "unit": "g N m-2"},
                     "CNVOL": {"name": "Accumulated Total Nitrogen Volatilized", "unit": "g N m-2 d-1"},
                     "CNLEACH": {"name": "Accumulated Nitrogen Leaching", "unit": "g N m-2 d-1"},
                     "CNDNIT": {"name": "Accumulated Denitrification", "unit": "g N g-1 H20 day-1"},
                     "CNUP": {"name": "Accumulated Nitrogen Uptake", "unit": "g N m-2"},
                     "CNSOL": {"name": "Accumulated Soluble Nitrogen", "unit": "g N m-2"}
                     }
        rename_dict = {"TE99": "TE", "ET99": "ETa", "EET99": "EET",
                       "CTR": "CTRa", "CSEVP": "SEVPa", "HI99": "HI"}
        graph_y_vars = [['WGRN', 'WVEG', 'HI99'],
                        ["CTR", "CSEVP", "ET99", "CPET"],
                        ['ATSW', 'CRAIN', 'CIRGW', 'WSTORG'],
                        ['CTR', 'CE', 'CRAIN', 'CIRGW', 'CRUNOF'],
                        ['CNMIN', 'CNFERT', 'CNVOL', 'CNLEACH', 'CNDNIT'],
                        ['CNSOL', 'CNUP', 'CNFERT']]
        for y_var in graph_y_vars:
                ...
                if isinstance(y_var, list):
                    ...
                    title_name_lst = []
                    for x in y_var:
                        if x in list(rename_dict.keys()):
                            xname = name_dict.get(x).get("name")
                            fname_entry = rename_dict.get(x)
                        else:
                            xname = name_dict.get(x).get("name")
                            fname_entry = x
                        xunit = name_dict.get(x).get("unit")
                        fname_lst.append(fname_entry)
                        title_name_lst.append("{} ({})".format(xname, xunit))
                    fname = '_'.join(fname_lst)
                    title_name = ' & '.join(title_name_lst)
                    title = 'Scenario: {}<br>Simulation Year(s): {}'.format(self.scenario_name, sim_y)
                    yaxis_title = ' &<br>'.join(title_name_lst)
                    data = []
                    for y in y_var:
                        y_graph = df[y].values.tolist()
                        if y in list(rename_dict.keys()):
                ...
                self.summary_graphs_ids.append('{}_{}'.format(self.scenario_name, fname))
    ...
    def write_graph_image_daily(self, basic_path):
        ...
        for i, f in zip(self.daily_graphs_ids, self.daily_graphs_output):
            output_path = os.path.join(basic_path, 'graph_images', 'daily',
                                       '{}.png'.format(i))
            pio.write_image(f, output_path, width=1500, height=1000)
